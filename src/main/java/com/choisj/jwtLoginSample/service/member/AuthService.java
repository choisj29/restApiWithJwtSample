...
import com.choisj.jwtLoginSample.service.redis.RedisService;
import com.choisj.jwtLoginSample.service.token.JwtTokenProvider;
...
public class AuthService {
    ...
    @Transactional
    public AuthDto.TokenInfo login(AuthDto.LoginMemberDto loginDto) {
        ...
        log.info("getAuthorities(authentication){}",getAuthorities(authentication));
        return generateToken(SERVER, authentication.getName(), getAuthorities(authentication));
    }

    // AccessToken 만료일자만 초과한 유효한 토큰인지 검사
    ...
    @Transactional
    public AuthDto.TokenInfo reissue(String requestAccessTokenInHeader, String requestRefreshToken) {
        String requestAccessToken = resolveToken(requestAccessTokenInHeader);
        ...
        // 요청된 RefreshToken의 유효성 검사 & Redis에 저장되어 있는 RefreshToken와 같은지 비교
        ...
        AuthDto.TokenInfo tokenDto = jwtTokenProvider.createToken(principal, authorities);
    ...
    // 토큰 발급
    @Transactional
    public AuthDto.TokenInfo generateToken(String provider, String loginId, String authorities) {
        // RefreshToken 이미 있을 경우
        ...
        AuthDto.TokenInfo tokenDto = jwtTokenProvider.createToken(loginId, authorities);
    ...
    public void logout(String requestAccessTokenInHeader) {
        log.info("logout AuthService in, requestAccessTokenInHeader is {}",requestAccessTokenInHeader);
        ...
        redisService.setValuesWithTimeout(requestAccessToken,
                "logout",
                expiration);
        log.info("refreshTokenInRedis {}", principal);
        log.info("expiration {}", principal);
    }
}
